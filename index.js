...
import os from 'os' // ← MEJORA: importado para que funcione autocleartmp
...
if (!opts['test']) {
    if (global.db) setInterval(async () => {
        ...
        if (opts['autocleartmp'] && (global.support || {}).find) {
            ...
            tmp.forEach((filename) => spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])) // ← MEJORA: cp.spawn → spawn (ya está importado)
        }
    }, 30 * 1000);
}
...
global.reloadHandler = async function(restatConn) {
    ...
    // ← MEJORA: Eliminado código muerto/redundante (currentDateTime y messageDateTime no se usaban y siempre ejecutaban lo mismo)
...
// Tmp cleaner (ya mejorado con fs.promises en mente, pero mantenido sync para compatibilidad)
setInterval(async () => {
